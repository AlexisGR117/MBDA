...
SELECT band_name FROM band
    WHERE extractvalue(Informacion,'/Informacion/Discografia/@ventas') =
        extractvalue(Informacion,'max(/Informacion/Discografia/*/@ventas)]');
...
-- Co
-- Presenta todos los datos de la banda, el nombre bdel músico de contacto y el nombre de cada uno de sus interpretes
DECLARE
CURSOR bandas(banda VARCHAR2) IS
    SELECT band_no, band_name, band_type, band_home, b_date, band_contact, m_name
        FROM band JOIN musician ON (m_no = band_contact)
            WHERE band_name = banda;
BEGIN
    FOR bnd in bandas('ROP')
        LOOP
        DBMS_OUTPUT.PUT_LINE(bnd.band_no ||' '|| bnd.band_name ||' '|| bnd.band_type ||' '|| 
        bnd.band_home ||' '|| bnd.b_date ||' '|| bnd.band_contact ||' '|| bnd.m_name);
        END LOOP;
END;   
...
-- TuplaOk
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('ROP', 5, 'classical', 11);
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('AASO', 6, 'classical', 10);
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('The J Bs', 8, 'jazz', 12);
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('BBSO', 9, 'classical', 21);
-- TuplaNoOk
-- No debe aceptar mas de dos palabras en el tipo de banda
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('ROP', 5, 'clas rock', 11);
-- Debe tener al menos una letra
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('#', 5, 'classical', 10);

-- AccionesOk
DELETE FROM band WHERE band_no = 7;

-- DisparadoresOk
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('The left Overs', 2, 'jazz', 8);
INSERT INTO band (band_name, band_home, band_type, band_contact) VALUES ('Somebody Loves this', 1, NULL, 6);
UPDATE band SET band_type = 'jazz' WHERE band_name = 'classical';
-- DisparadoresNoOk
-- Insertar el número de banda
INSERT INTO band (band_no, band_name, band_home, band_type, band_contact) VALUES (7, 'Swinging strings', 4, NULL, 7);
-- Insertar la fecha de banda
INSERT INTO band (band_name, band_home, band_type, b_date, band_contact) VALUES ('The Rest', 9, 'jazz', 17/02/2021, 16);
-- Actualizar un atributo de banda que no sea el tipo
UPDATE band SET band_name = 'Nirvana' WHERE band_name = 'The Rest';

-- CRUDE

CREATE OR REPLACE PACKAGE PC_bands IS
    PROCEDURE ad (name IN VARCHAR2, home IN NUMBER, type IN VARCHAR2,
                  contact IN NUMBER);
    PROCEDURE up_type (band IN NUMBER, type IN VARCHAR2);
    PROCEDURE up_ad_player (band IN NUMBER, xplayer IN NUMBER);
    FUNCTION co_band (banda IN NUMBER) RETURN SYS_REFCURSOR;
    FUNCTION co_players (banda IN NUMBER) RETURN SYS_REFCURSOR;
...
CREATE OR REPLACE PACKAGE BODY PC_bands IS
    ...
    BEGIN
        INSERT INTO band (band_name, band_home, band_type, band_contact)
            VALUES (name, home, type, contact);
        COMMIT;
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
                RAISE_APPLICATION_ERROR(-20001, 'Error al insertar la banda');
    END ad;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
                RAISE_APPLICATION_ERROR(-20002, 'Error al actualizar el tipo de la banda');
    END up_type;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
                RAISE_APPLICATION_ERROR(-20003, 'Error al insertar el interprete');
    END up_ad_player;
    ...
    BEGIN
        OPEN s_cursor FOR
            SELECT band_no, band_name, band_type, band_home, b_date, m_name AS Contacto
                FROM band JOIN musician ON (m_no = band_contact)
                    WHERE band_no = banda;
        RETURN s_cursor;
    END;
    ...
    BEGIN
        OPEN s_cursor FOR
            SELECT m_name AS Musicos
                FROM plays_in JOIN performer ON (player = perf_no) JOIN musician ON (m_no = perf_is)
                    WHERE band_id = banda;
        RETURN s_cursor;
    END;
    ...
    IS
    BEGIN
        DELETE FROM band WHERE (band = band_no);
        COMMIT;
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
                RAISE_APPLICATION_ERROR(-20004, 'Error al eliminar la banda');
    END de;
    ...
    BEGIN
        OPEN s_cursor FOR
            SELECT band_name AS Banda, c_title AS Composicion
            FROM band JOIN (performance JOIN (composition JOIN (has_composed
                JOIN (composer JOIN musician ON (composer.comp_is = musician.m_no))
                ON (composer.comp_no = has_composed.cmpr_no))
                ON (composition.c_no = has_composed.cmpn_no))
                ON (composition.c_no = performance.performed))
                ON (band.band_no = performance.gave)
             WHERE comp_no = composer;
        RETURN s_cursor;
    END co_byComposer;
    ...
    BEGIN
        OPEN s_cursor FOR
            SELECT band_name AS Banda, COUNT(band_name) AS Musicos FROM plays_in
                JOIN band ON (band.band_no = plays_in.band_id)
                GROUP BY band_name
                HAVING COUNT(band_name) > (SELECT AVG(Musicos)
                                  FROM (SELECT COUNT(band_name) AS Musicos
                                        FROM plays_in JOIN band ON (band.band_no = plays_in.band_id)
                                        GROUP BY band_name));
        RETURN s_cursor;
    END co_theBigOnes;
END PC_bands;
